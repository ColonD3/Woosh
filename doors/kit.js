...
// palette
const P={oak:'#c49464',oakD:'#8e6038',oakDD:'#5e3d24',pine:'#d8b07a',iron:'#6c6f73',ironD:'#56595f',stone:'#b9b1a4',stoneD:'#8f877b',rope:'#cdb27c',clay:'#a88f7c',ash:'#3a302a'};
...
function wall(L,H,T,concrete,sd){reseed(sd||1);const woodP=[],stoneP=[],ironP=[];const y0=-H/2;
 ...
 while(y<top-.12){const ch=Math.min(rr(.32,.42),top-y);let x=x0+(row%2?-rr(.1,.25):0);
  while(x<x1-.05){let w=rr(.42,.78);const xa=Math.max(x,x0),xb=Math.min(x+w,x1);if(xb-xa>.12){const tint=[P.stone,'#aaa295','#c6bdae','#9f978b'][Math.floor(R()*4)];
